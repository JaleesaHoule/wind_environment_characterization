...
def load_data_from_directory(data_directory): 
    ...
    number_fill_bytes = 110  # guess 
    number_data_records = 100 # guess
    ...
    for filename in filenames:
        data_buff = load_buffers_from_file(filename, data_type, number_fill_bytes, number_data_records)
        if df1 is None:
            df1 = pandas.DataFrame(data_buff)
        else:
            df1 = pandas.concat([df, pandas.DataFrame(data_buff)], ignore_index=True)
    ...
    for filename in filenames:
        data_buff = load_buffers_from_file(filename, data_type, number_fill_bytes, number_data_records)
        if df2 is None:
            df2 = pandas.DataFrame(data_buff)
        else:
            df2 = pandas.concat([df, pandas.DataFrame(data_buff)], ignore_index=True)
    ...
    for filename in filenames:
        data_buff = load_buffers_from_file(filename, data_type, number_fill_bytes, number_data_records)
        if df3 is None:
            df3 = pandas.DataFrame(data_buff)
        else:
            df3 = pandas.concat([df, pandas.DataFrame(data_buff)], ignore_index=True)
